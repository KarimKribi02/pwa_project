...
export default function CheckoutPage() {
  const { cartItems, submitOrder, clearCart } = useCartSync();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      id_utilisateur: undefined as string | undefined,
      ...
      largeur: width || undefined,
      longueur: length || undefined,
      couleur: order.customization?.finish || undefined,
      type_bois: order.customization?.wood || undefined,
      prix_total: order.price,
      id_produit: order.product_id
    ...
    try {
      ...
      setIsQueued(result.queued);
      setEmailWarning(
        result.emailNotification && !result.emailNotification.sent
          ? result.emailNotification.message
          : null,
      );
      setIsSuccess(true);
      await clearCart();
    ...
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center p-8 text-center">
        <div>
          ...
          <Link href="/catalog" className="text-accent font-bold hover:underline">
      ...
      <AnimatePresence>
        {!isSuccess ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
              <div className="space-y-12">
                <header>
                  <Link
                    href="/catalog"
                    className="flex items-center gap-2 text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-8 hover:text-primary transition-colors"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="bg-amber-50/55 border border-amber-200 p-5 rounded-2xl flex items-start gap-3 text-amber-800 shadow-sm">
                      ...
                      <div>
                        ...
                        <p className="text-xs leading-relaxed font-medium">
                          Veuillez remplir tous les champs obligatoires pour finaliser votre commande.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-primary/5 space-y-6">
                      ...
                      <div key={field} className="space-y-2">
                        ...
                        <input
                          ...
                          className={`w-full bg-[#fcf9f3] px-6 py-4 rounded-xl outline-none border transition-all text-sm font-semibold text-primary focus:ring-2 focus:ring-[#2D5A27]/5 focus:border-[#2D5A27] ${
                            errors[field] ? 'border-red-300 bg-red-50/30' : 'border-stone-200/60'
                          }`}
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        rows={3}
                        className={`w-full bg-[#fcf9f3] px-6 py-4 rounded-xl outline-none border transition-all text-sm font-semibold text-primary resize-none ${
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        rows={2}
                        className="w-full bg-[#fcf9f3] px-6 py-4 rounded-xl outline-none border border-stone-200/60 text-sm font-semibold text-primary resize-none"
                  ...
                  <button
                    disabled={loading}
                    className="w-full bg-primary hover:bg-[#22441D] text-white py-5 rounded-2xl font-bold flex items-center justify-center gap-4 transition-all shadow-xl shadow-primary/10 disabled:opacity-50"
                  >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/50 border border-primary/5 p-6 rounded-3xl flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-stone-400">À la Livraison</p>
                    </div>
                  </div>
                  <div className="bg-white/50 border border-primary/5 p-6 rounded-3xl flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-stone-400">Fait à Marrakech</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="sticky top-32">
                <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-primary/5">
                  <h3 className="text-2xl font-serif text-primary border-b border-primary/5 pb-6 mb-8 italic">
                    Votre Sélection
                  </h3>
          ...
          >
            ...
            {isQueued ? (
              <div className="max-w-md mx-auto bg-[#fdfaf3] border border-amber-200/50 p-6 rounded-[2rem] text-left mb-10 flex items-start gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] leading-relaxed font-medium text-stone-500">
                    Votre commande est enregistrée localement et sera transmise automatiquement à
                    l&apos;atelier dès le retour du réseau.
                  </p>
                </div>
              </div>
            ) : (
              <p className="text-stone-500 mb-12 leading-relaxed text-sm">
                Votre commande a été reçue avec succès par nos artisans.
              </p>
            )}

            {emailWarning && (
              <div className="max-w-md mx-auto bg-red-50 border border-red-200/80 p-6 rounded-[2rem] text-left mb-8 flex items-start gap-4">
                <div className="p-3 bg-red-500/10 rounded-2xl shrink-0 text-red-600">
                  <MailWarning size={20} />
                </div>
                <div className="space-y-1">
                  <h4 className="text-[10px] font-extrabold uppercase tracking-widest text-red-800">
                    Email non envoyé
                  </h4>
                  <p className="text-[11px] leading-relaxed font-medium text-red-700/90">
                    {emailWarning}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-2xl border-2 border-secondary/20 relative overflow-hidden mb-12">
              <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-3">
                {isQueued ? 'Référence locale' : 'Votre Code de Suivi'}
              </p>
              <div className="flex items-center justify-center gap-4">
                <span className="text-4xl font-serif text-primary font-bold tracking-tight">
                  {trackingCode}
                </span>
                <button
            ...
            <div className="space-y-4">
              <Link
                href="/suivi"
                className="block bg-primary text-white py-5 rounded-full font-bold shadow-xl shadow-primary/20"
              >
                Suivre ma Fabrication
              </Link>
              <Link href="/" className="block text-stone-400 font-bold text-xs uppercase tracking-widest">
                Retour à l&apos;Accueil
              </Link>
            </div>
          </motion.div>
